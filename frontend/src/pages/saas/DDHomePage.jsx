...
export default function DDHomePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-slate-900">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200/80">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-[#0096DB] uppercase tracking-wider">
        ...
        {user && (
          <div className="shadcn-card px-4 py-3 flex items-center gap-4 shrink-0 bg-white/80 backdrop-blur-xl border border-white/90 shadow-glass rounded-lg">
            <div className="text-right">
              <span className="text-xs text-slate-500 block">可用尽调额度</span>
              <span className="text-xl font-bold text-[#0096DB] font-mono leading-none">
            ...
            <button 
              ...
              className="shadcn-button-outline text-xs py-1.5 px-3 hover:border-[#0096DB] hover:text-[#0096DB] shadow-xs"
            >
      ...
      {/* 额度不足提示 Banner */}
      {!hasQuota && (
        <div className="mt-6 p-4 rounded-lg bg-amber-50/80 backdrop-blur-sm border border-amber-200/80 text-xs text-amber-950 flex items-start justify-between gap-3 shadow-xs">
      ...
      <div className="mt-8 space-y-6">
        ...
        {/* 1. 目标企业主体与统一信用代码 */}
        <div className="shadcn-card p-6 bg-white/80 backdrop-blur-xl space-y-5 border border-white/90 shadow-glass rounded-lg">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div>
              <label className="text-base font-bold text-slate-900 flex items-center gap-2">
                <Building2 className="w-4 h-4 text-[#0096DB]" />
                1. 目标企业主体与统一社会信用代码
              </label>
            ...
            <span className="shadcn-badge-secondary font-mono bg-cyan-50 text-[#0084c2] border border-cyan-200/60">
              STEP 01
            </span>
          </div>

          {/* 突出的双核心输入区域 */}
          <div className="p-5 bg-white/60 backdrop-blur-md rounded-lg border border-slate-200/70 space-y-4 shadow-2xs">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-900 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-400 font-normal">须与营业执照一致</span>
                </label>
                <div className="flex items-center bg-white/90 border border-slate-200/90 rounded-md px-3.5 py-2.5 focus-within:border-[#0096DB] focus-within:ring-2 focus-within:ring-[#0096DB]/20 transition-all shadow-2xs">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-900 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-400 font-normal">18位大写英数</span>
                </label>
                <div className="flex items-center bg-white/90 border border-slate-200/90 rounded-md px-3.5 py-2.5 focus-within:border-[#0096DB] focus-within:ring-2 focus-within:ring-[#0096DB]/20 transition-all shadow-2xs">
        ...
        {/* 2. 企业数据授权全量接入清单 */}
        <div className="shadcn-card p-6 bg-white/80 backdrop-blur-xl space-y-4 border border-white/90 shadow-glass rounded-lg">
          ...
          <div className="space-y-3 pt-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs">
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <Building className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">市监工商主体照面与董监高治理</span>
              </div>
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <Users className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">股权出资穿透与最终实控人图谱</span>
              </div>
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <Scale className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">司法涉诉裁判、失信与行政处罚</span>
              </div>
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <ShieldCheck className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">经营异常名录与动产抵质押排查</span>
              </div>
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <Truck className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">供应链客商分布与产业经营态势</span>
              </div>
              <div className="p-3 bg-white/70 backdrop-blur-sm border border-slate-200/70 rounded-md flex items-center gap-2.5 text-slate-800 shadow-2xs hover:border-[#0096DB]/40 transition-colors">
                <Bookmark className="w-4 h-4 text-[#0096DB] shrink-0" />
                <span className="font-semibold">企业全景尽调报告全文与原件下载</span>
              </div>
            </div>
            
            <div className="p-3.5 bg-cyan-50/50 backdrop-blur-sm border border-cyan-100/70 rounded-md text-xs text-slate-800 flex items-center gap-2.5">
              <QrCode className="w-4 h-4 text-[#0096DB] shrink-0" />
        ...
        <div className="pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            disabled={submitting}
            className="shadcn-button-primary px-8 py-3 text-sm font-semibold flex items-center gap-2"
          >
